...
def multiplayer():
    ...
    while (True):
        ...
        if snake.get_head_position_p1() in snake.positions_p2:
            snake.reset()
            snake.overall_p2 += 1
            screen.blit(surface, (0,0))
            show_scores(False)
        ...
        if snake.get_head_position_p2() in snake.positions_p1:
            snake.reset()
            snake.overall_p1 += 1
            centre_display("Player 1's point", True)
        ...
        snake.draw_p1(surface)
        snake.draw_p2(surface)
        food.draw(surface)
        ...
        def show_scores(inverted):
            if inverted:
                ...
                score = myfont.render("(" + str(snake.score_p1) + ")" + 
        ...
        def centre_display(text, sound):
            ...
            for i in range(0, 8):
                if (i % 2) == 0:
                    drawGrid(surface, True)
                    # blit screen
                    screen.blit(surface, (0,0))
                    # display scores
                    show_scores(True)
                    # text render
                    status = mylargefont.render(text, 1, board_light)
                    ...
                    show_scores(False)
                    # text render
                    status = mylargefont.render(text, 1, snake_color1)
                    status_rect = status.get_rect(center=(screen_width/2, screen_height/2))
                    screen.blit(status, status_rect)
                    pygame.display.update()
                    # screen pause
